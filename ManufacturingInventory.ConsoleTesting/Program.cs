...
namespace ManufacturingInventory.ConsoleTesting {
    public class Program {
        ...
        public static void UseCaseTesting_Backend() {
            ...
            if (user != null) {
                ...
                IRepository<Attachment> attachmentRepository = new AttachmentRepository(context);
                IRepository<PartInstance> partInstanceRepository = new PartInstanceRepository(context);
                IEntityProvider<Transaction> transactionProvider= new TransactionProvider(context);
                IEntityProvider<Location> locationProvider = new LocationProvider(context);
                IEntityProvider<Category> categoryProvider = new CategoryProvider(context);
                IEntityProvider<PartInstance> provider = new PartInstanceProvider(context);
                IRepository<BubblerParameter> bubblerRepository = new BubblerParameterRepository(context);
                IUnitOfWork unitOfWork = new UnitOfWork(context);

                //PartInstanceDetailsEdit instanceEdit = new PartInstanceDetailsEdit(partInstanceRepository, attachmentRepository, transactionProvider, categoryProvider, locationProvider,bubblerRepository, unitOfWork);
                var instancesTemp = provider.GetEntityList(e => e.PartId == 2);
                var catTemp = categoryProvider.GetEntityList();
                var locTemp = locationProvider.GetEntityList();

                ObservableCollection<PartInstance> instances=new ObservableCollection<PartInstance>(instancesTemp);
                ObservableCollection<Condition> conditions = new ObservableCollection<Condition>(catTemp.OfType<Condition>());
                ObservableCollection<PartType> tpyes = new ObservableCollection<PartType>(catTemp.OfType<PartType>());
                ObservableCollection<Location> locations = new ObservableCollection<Location>(locTemp);

                var location = locations.FirstOrDefault(e => e.Id == 4);
                var condition = conditions.FirstOrDefault(e => e.Id == 2);
                


                var instance = instances.FirstOrDefault(e => e.Id == 4);
                instance.BubblerParameter.Measured = 2200;
                instance.Name = "TMA-Edit";
                instance.ConditionId = condition.Id;
                instance.LocationId = location.Id;



                bubblerRepository.Update(instance.BubblerParameter);
                partInstanceRepository.Update(instance);
                var count=unitOfWork.Save();
                //PartInstanceDetailsEditInput input = new PartInstanceDetailsEditInput(instance, false);
        ...
        public static async Task UseCasePartInstanceTesting() {
            Console.WriteLine("Testing Service Started:");
            IUserService userService = new UserService();
            using var context = new ManufacturingContext();
            ...
            if (user != null) {
                ...
                PartInstanceDetailsEdit instanceEdit = new PartInstanceDetailsEdit(partInstanceRepository, attachmentRepository, transactionProvider, categoryProvider, locationProvider,bubblerRepository, unitOfWork);
                var instancesTemp = await provider.GetEntityListAsync(e => e.PartId == 2);
                var catTemp = await instanceEdit.GetCategories();
                var locTemp = await instanceEdit.GetLocations();
                ...
                var location = locations.FirstOrDefault(e => e.Id == 4);
                var condition = conditions.FirstOrDefault(e => e.Id == 1);
                var instance = instances.FirstOrDefault(e => e.Id == 7);

                instance.BubblerParameter.Measured = 2100;
                instance.Name = "TMA-Edit-7";
                instance.ConditionId = condition.Id;
                instance.LocationId = location.Id;
                PartInstanceDetailsEditInput input = new PartInstanceDetailsEditInput(instance, false);
                var output=await instanceEdit.Execute(input);
                if (output.Success) {
                    Console.WriteLine(output.Message);
                } else {
                    Console.WriteLine("Error Updating Part");
                }
            } else {
                Console.WriteLine("Error: UserService Failed");
            }
        ...
        public static async Task<int> Main(string[] args) {
            await UseCasePartInstanceTesting();
            return 1;
        }
